...
const buscarCidades = ( db, uf) => {
   return db.filter ( estado => estado.sigla == uf)[0].cidades
          .map ( cidades => cidades.nome );
}
...
const mostrarCidades = () => {
   ...
   const cidades = buscarCidades ( bdCidades, estado.toUpperCase() );
   $conteiner.innerHTML = criarLinhas ( cidades );
}

const criarLinhas = cidades => {
   return cidades.reduce ( (acc, cidade, i) => 
      `${acc}
